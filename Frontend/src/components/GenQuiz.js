...
function GenQuiz({ data, handleSubmit, handleAdd }) {
		...
		<div className="d-flex flex-grow-1 w-100">
			<form ref={formRef} id="form" onSubmit={(e) => { handleAdd(e) }} className="d-flex flex-column bd-highlight ml-5 my-3 mx-5" style={{ width: "50%" }}>
				...
				<div className="d-flex justify-content-around">
					<button type="submit" className="btn btn-dark mt-3 " style={{ width: "max-content", position: "relative", top: "7px" }}>
						Add
					</button>
				</div>
			</form>
			<button onClick={handleClick} className="btn btn-dark mt-3" style={{ width: "max-content", position: "absolute", left: "-2rem", top: "34rem" }}>Add to Resources</button>
			<div className="d-flex my-6 justify-content-space-around  align-items-start">
				<button onClick={(e) => { handleSubmit(e) }} className="btn btn-dark mt-3 " style={{ width: "max-content" }}>
					Send
				</button>
			</div>
